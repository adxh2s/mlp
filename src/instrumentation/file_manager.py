...
class FileManager:
    ...
    def make_timestamp_name(self, src: str | Path) -> str:
        """Return a timestamped filename YYYYMMDD_HHMMSS_."""
        p = Path(src)
    ...
    def pick_input_file(
        ...
        """
        Retourne le fichier d'entrée:
        - Si preferred_filename est fourni et existe dans in_dir: le retourne.
        - Sinon, liste les fichiers par extensions et renvoie le plus récent (mtime desc).
        """
        base = Path(in_dir)
