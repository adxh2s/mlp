...
class GeneralOrchestrator(LoggerMixin):
    ...
    def __init__(
        ...
        # Messages
        self.msg_orch = message_orchestrator or MessageOrchestrator(
            self.cfg_mgr, logger_manager=self.lm
        )
        ...
        if ctx is None:
            ...
            self.ctx = {
                ...
                "project_dir": str(Path(out_dir) / self.cfg.project.name),
            }
        else:
            self.ctx = ctx
        ...
        if orchestrators.file and orchestrators.file.enabled:
            try:
                file_orch = FileOrchestrator(
                    orchestrators.file, logger_manager=self.lm, ctx=self.ctx
                )
    ...
    def _run_ml_orchestrators(
        self, X: pd.DataFrame, y: Optional[pd.Series], results: dict[str, Any]
    ) -> dict[str, Any]:
        orchestrators = self.cfg.orchestrators
            ...
            try:
                ...
                if p_out_cfg:
                    ...
                    if p.is_absolute():
                        ...
                        root_dir = (
                            getattr(self, "root_dir", None) or Path(self.project_dir).parent.parent
                        )
                ...
                self.msg_orch.emit(
                    DOMAIN, PIPELINES_ORCHESTRATOR_FAILED, level="error", error=str(exc)
                )
        ...
        if orchestrators.report.enabled:
            try:
                ...
                results[KEY_REPORT] = rep.run(
                    results.get(KEY_EDA, {}), results.get(KEY_PIPELINES, {"results": []})
                )
            except Exception as exc:  # noqa: BLE001
                self.msg_orch.emit(
                    DOMAIN, REPORT_ORCHESTRATOR_FAILED, level="error", error=str(exc)
                )

        self.msg_orch.emit(
            DOMAIN, GENERAL_DONE, report_artifacts=results.get(KEY_REPORT, {}).get("artifacts")
        )
        return results
